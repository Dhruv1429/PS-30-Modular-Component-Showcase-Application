...
const DataTableDemo = () => {
  ...
  // 1. The State Architecture (Restored!)
  ...
  // 3. Optimized Rendering & Dynamic Route Click Handler
        ...
        <table className="ps-table">
          ...
          <tbody>
            {dataState.status === 'idle' && (
              <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px', color: '#64748b' }}>Click "Fetch Live Data" to consume the API.</td></tr>
            )}
            
            {dataState.status === 'loading' && (
              <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px' }}>
                <span style={{ color: '#2563eb', fontWeight: '500' }}>Connecting to database...</span>
              </td></tr>
            )}

            {dataState.status === 'error' && (
              <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px' }}>
                <span className="status-badge status-error">{dataState.error}</span>
              </td></tr>
            )}
